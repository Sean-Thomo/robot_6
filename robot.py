...
def valid_command(command):
    ...
    if command_name.lower() == 'replay':
        ...
        if len(arg1.strip()) == 0:
            return True
        elif ((arg1.lower().find('top') > -1 or arg1.lower().find('left') > -1 or arg1.lower().find('right') > -1 or arg1.lower().find('bottom') > -1)) and (
            len(arg1.lower().replace('top', '').replace('left','').replace('right','').replace('bottom','').strip())) == 0:
            return True
        else:
            range_args = arg1.replace('top', '').replace('left','').replace('right','').replace('bottom','')
            if is_int(range_args):
                return True
            else:
                range_args = range_args.split('-')
...
MAZERUN - will let the robot figure out what commands to use to get to the top of the screen
MAZERUN TOP - will let the robot figure out what commands to use to get to the top of the screen
MAZERUN RIGHT - will let the robot figure out what commands to use to get to the right of the screen
MAZERUN LEFT - will let the robot figure out what commands to use to get to the left of the screen
MAZERUN BOTTOM - will let the robot figure out what commands to use to get to the bottom of the screen
"""
...
def call_command(command_name, command_arg, robot_name):
    ...
    elif (command_name == 'mazerun' and command_arg == '') or (command_name == 'mazerun' and command_arg == 'top'):
        return mazerun.mazerun()
    elif command_name == 'mazerun' and command_arg == 'left':
        return mazerun.mazerun_left()
    elif command_name == 'mazerun' and command_arg == 'right':
        return mazerun.mazerun_right()
    elif command_name == 'mazerun' and command_arg == 'bottom':
        return mazerun.mazerun_bottom()
    return False, None
